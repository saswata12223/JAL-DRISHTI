...
class RiskDecisionEngine:
    ...
    def get_latest_decisions(
        ...
        """Retrieves and evaluates the latest state across monitored points in Uttarakhand."""
        pred_file = self.data_dir / "ml" / "flood_risk_predictions.parquet"
        if not pred_file.exists():
            return []

        df_pred = pd.read_parquet(pred_file)
        df_latest = df_pred.sort_values("timestamp_utc").groupby("spatial_id", as_index=False).last()
        ...
        for _, r in df_latest.iterrows():
            ...
            wl_m = float(r["water_level_m"]) if pd.notna(r.get("water_level_m")) else None
            warn_m = float(r["warning_level_m"]) if pd.notna(r.get("warning_level_m")) else None
            dang_m = float(r["danger_level_m"]) if pd.notna(r.get("danger_level_m")) else None
            hfl_m = float(r["hfl_m"]) if pd.notna(r.get("hfl_m")) else None

            # Station context
            stn_info = self._cwc_thresholds_cache.get(sp_id)
            if stn_info:
                warn_m = warn_m or stn_info["warning_level_m"]
                dang_m = dang_m or stn_info["danger_level_m"]
                hfl_m = hfl_m or stn_info["hfl_m"]

            cwc_status, alert_stage = self.evaluate_cwc_threshold_state(wl_m, warn_m, dang_m, hfl_m)
            ...
            decisions.append(
                RiskDecisionResponse(
                    ...
                    latitude=float(r["latitude"]),
                    longitude=float(r["longitude"]),
                    model_name=str(r.get("model_name", "XGBoost_PPT_Upgraded")),
                    model_version=str(r.get("model_version", "6.1.0")),
                    flood_probability=prob,
                    ml_risk_class=ml_class,
                    ml_decision_threshold=self.DECISION_THRESHOLD,
                    water_level_m=wl_m,
                    ...
                    cwc_threshold_status=cwc_status,
                    official_alert_stage=alert_stage,
                    is_gauge_offline=True if wl_m is None else False,
                    rainfall_1h_mm=r1h,
                    rainfall_3h_mm=r3h,
                    soil_saturation_index=ssi,
                    scs_direct_runoff_q_mm=scs_q,
    ...
    def get_active_alerts(self) -> RiskAlertsResponse:
        """Retrieves active high-priority and critical alerts across Uttarakhand."""
        decisions = self.get_latest_decisions(limit=10000)
