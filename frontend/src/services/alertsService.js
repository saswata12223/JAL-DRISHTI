...
export async function loadAlerts() {
  ...
  try {
    ...
    if (valid && records.length > 0) {
      ...
      // Deduplicate by stationId / stationName to prevent duplicate alert cards
      ...
      for (const r of records) {
        ...
        const key = r.stationId || r.stationName || r.id;
      ...
      devWarning(`Adopting validated live alert layer (${uniqueRecords.length} unique alerts).`);
